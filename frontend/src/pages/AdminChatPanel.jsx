  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        // Listen for new incoming chat requests
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          ...
          {filtered.map((s) => (
            <button key={s.id} type="button" onClick={() => openSession(s)}
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500">{s.refType} #{s.refId}</p>
            </button>
          ))}
        </div>
      </div>
          ...
          <>
            <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500">@{activeSession.username} · {activeSession.refType} #{activeSession.refId}</p>
              </div>
              <div className="flex items-center gap-2">
                ...
                {activeSession.status === 'ACTIVE' && (
                  <button type="button" onClick={() => closeSession(activeSession.id)}
                    className="px-3 py-1.5 bg-slate-500 hover:bg-slate-600 text-white text-xs font-semibold rounded-lg transition-colors">
                    Close
                  </button>
                )}
              </div>
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${
                      isMe ? 'bg-amber-500 text-white rounded-br-sm' : 'bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-white rounded-bl-sm'
                    }`}>
            ...
            {activeSession.status === 'ACTIVE' && (
              <form onSubmit={sendText} className="px-4 py-3 border-t border-slate-100 dark:border-slate-700 flex items-center gap-2">
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()}
                  className="p-2 text-slate-400 hover:text-amber-500 transition-colors" title="Send image">
                  🖼️
                </button>
                ...
                <button type="submit" disabled={!text.trim()}
                  className="px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-amber-300 text-white font-semibold rounded-xl text-sm transition-colors">
                  Send
                </button>
              </form>
            )}
